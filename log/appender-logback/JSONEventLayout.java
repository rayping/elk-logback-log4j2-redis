...
public class JSONEventLayout extends LayoutBase<ILoggingEvent> {
	...
	public synchronized String doLayout(ILoggingEvent event) {
		...
		this.buf.append(',');
		appendKeyValue(this.buf, "logdate", this.df.format(new Date(event.getTimeStamp())), null);
		...
		if (this.locationInfo) {
			...
			if ((callerDataArray != null) && (callerDataArray.length > this.callerStackIdx)) {
				...
				appendKeyValue(this.buf, "class", immediateCallerData.getClassName(), null);
				this.buf.append(',');
				appendKeyValue(this.buf, "method", immediateCallerData.getMethodName(), null);
				this.buf.append(',');
				appendKeyValue(this.buf, "file", immediateCallerData.getFileName(), null);

				this.buf.append(',');
				appendKeyValue(this.buf, "line", Integer.toString(immediateCallerData.getLineNumber()), null);
		...
		if (this.properties) {
			...
			if ((propertyMap != null) && (propertyMap.size() != 0)) {
				...
				while (i.hasNext()) {
					...
					appendKeyValue(this.buf, entry.getKey(), entry.getValue(), null);
					if (i.hasNext()) {
						this.buf.append(',');
					}
				}
