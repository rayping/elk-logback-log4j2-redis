...
import static com.yichuan.wuzhenpay.openapi.base.mvc.log.appender.Helpers.requireArgument;
...
import com.yichuan.wuzhenpay.openapi.base.mvc.log.GwsLoggerTypeEnum;
...
public class RedisAppender implements Appender {
    ...
    private RedisAppender(Builder builder) {
		...
		this.jsonlayout = new JSONEventLayout(builder.getSource(), builder.getType(), builder.getTags());
    }
    ...
    public void append(LogEvent event) {
		...
		if (State.STARTED.equals(state) && StringUtils.isNotBlank(loggerName)) {
			if (GwsLoggerTypeEnum.GWS.name().equals(loggerName)) {
				// String message = layout.toSerializable(event).toString();
				byte[] eventBytes = this.jsonlayout != null ? this.jsonlayout.doLayout(event)
						: this.layout.toByteArray(event);
				if (null != eventBytes && eventBytes.length > 0) {
    ...
    private void connect() {
		...
		if (StringUtils.isNotBlank(password)) {
			jedisPool = new JedisPool(poolConfig.getJedisPoolConfig(), host, port, connectionTimeoutMillis,
					socketTimeoutMillis, password, 0, // database
					null, // clientName
					false, // ssl
